...
if (isset($_GET['id'])) {
    try {
        ...
        if (!$subject) {
            $_SESSION['error_message'] = "រកមិនឃើញមុខវិជ្ជានេះទេ";
            header("Location: subject-list.php");
            exit();
        }
    } catch (PDOException $e) {
        $_SESSION['error_message'] = "Error fetching subject: " . $e->getMessage();
        header("Location: subject-list.php");
        exit();
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $stmt->execute([$subject_code, $subject_name, $teacher_id, $credits, $description, $id]);

        $_SESSION['success_message'] = "បានកែប្រែមុខវិជ្ជាដោយជោគជ័យ";
        header("Location: subject-list.php");
        exit();
                        ...
                        <div>
                            ...
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">កែប្រែមុខវិជ្ជា</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="credits" name="credits" 
                                               value="<?php echo htmlspecialchars($subject['credits']); ?>" min="0" required>
                                        <div class="invalid-feedback">
                                            សូមបញ្ចូលចំនួនឥណទាន
                                        </div>
                                    </div>
